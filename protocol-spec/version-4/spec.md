...
**Version:** 4.0.0
**Version Note:** Major — breaking changes to Room Advertisement prefix (`"LB1"` → `"LB4"`), HELLO payload format (`proto_version` field added), Client Disconnect Decision Tree (reconnect-before-recovery ordering, simplified `shouldEmit` derivation, `connectionFailureHandled` flag removed), and new normative mechanisms (Recovery Host Probe, protocol version negotiation, BLE availability check procedure) relative to v3.0.0.
**Revision Basis:** Consolidated from two independent proposals against v3.0.0, with four post-consolidation amendments applied. Finalized 2026-03-27.
**Status:** Draft

### Changelog

1. Client Disconnect Decision Tree (Section 13) step ordering reversed: reconnect attempted before host recovery for all transports; reconnect timeout escalates to recovery for Resilient transport (Section 7.1). Recovery Host Probe added to Section 8.2 for self-elected successors. Recovery Host Probe Duration (1.5s) added to Section 17. (Merged, amended)
2. Graceful migration roster cleanup: `BeginHostingMigratedSession` (Section 8.4) removes old host from roster as first step; `CompleteMigrationResume` (Section 8.5) `peers` field must not include old host. (Merged)
3. `shouldEmit` derivation simplified: device-identity guards (steps 3b/3c) removed from Section 13; callback-routing REQUIREMENT added above the decision tree. (Merged)
4. `connectionFailureHandled` flag removed from Section 13 and Section 6.3; replaced with platform-concern REQUIREMENT for dual-delivery guard. (Merged)
5. "First advertiser wins" convergence rule (Section 8.4 `OnScanResultDuringMigration` step 3) strengthened with normative REQUIREMENT applying to all migration paths. (Merged)
6. Protocol version negotiation: Room Advertisement prefix changed from `"LB1"` to `"LB"` followed by version digit (`"LB4"` for v4); HELLO payload extended with `proto_version` field; `join_rejected("incompatible_version")` added; `room_found` event gains `proto_version` field; `DecodeRoom` updated for version extraction and incompatibility flagging. (Merged, amended)
7. BLE availability check: new Section 6.0 `AssertBleAvailable()` procedure replaces undefined assertion in Sections 6.1, 6.2, 6.3; `"resetting"` state added to `radio` event; `ble_unavailable` error code added. (Merged)
8. Deferred: join-boundary enforcement for rooms flagged as incompatible — whether the native layer blocks `Join()` or delegates to HELLO validation. (Deferred)
...
| **HELLO** | A control Packet sent by a Client to a Host to initiate the join handshake. Payload includes `session_id`, `join_intent`, and `proto_version`. |
...
| **shouldEmit** | A boolean derived at the moment of client disconnection that controls whether the disconnect decision tree may emit events or initiate recovery procedures. Derived from `clientLeaving`. Captured before `StopClientOnly()` executes. |
...
- `proto_version` — A decimal integer string representing the client's major protocol version (e.g., `"4"`). For backward compatibility, a missing third field is treated as version 1.
...
| `incompatible_version` | The resolved protocol version does not match the host's protocol version. |
...
1. Call AssertBleAvailable("host"). If false, return.
...
**Scan()**
1. Call AssertBleAvailable("scan"). If false, return.
...
**Join(roomID)**
1. Call AssertBleAvailable("join"). If false, return.
...
8. On GATT connection failure (or post-connection setup failure from 7b/7c):
   ...
   b. If `reconnectJoinInProgress` is true:
        ...
        -> Resume reconnect scan. Do not call FailReconnect().
           ...
           another scan result.
        -> Return.
   c. If `migrationJoinInProgress` is true:
...
5. Encode and enqueue a HELLO control Packet (from=localPeerID, to=hostPeerID, type=`"hello"`, payload=`sessionId|joinIntent|protoVersion`).
...
2. Parse payload: extract *sessionId*, *joinIntent*, and *protoVersion* from the HELLO payload.
3. Validate admission:
   ...
   f. Resolve the client's protocol version: if the `proto_version` field is present in the HELLO payload, use its value; if the field is absent, treat the client's protocol version as 1 (per Section 4.3 backward-compatibility rule).
   g. If the resolved protocol version does not match the host's protocol version: send `join_rejected("incompatible_version")` to device, disconnect device, return.
...
The `peers` field in the `session_resumed` event MUST NOT include the old host. The old host should already have been removed from the roster by `BeginHostingMigratedSession` step 1 (for the successor) or by `BeginUnexpectedHostRecovery` step 3 (for all peers during unexpected recovery).
...
|-------|--------|---------|
| `room_found` | room_id, session_id, name, transport, peer_count, max, rssi, proto_version, incompatible | Scan discovers a Room |
...
| `radio` | state | Bluetooth radio state changed ("on", "off", "unauthorized", "unsupported", "resetting") |
...
Note: The `join_failed` event is a distinct event type (not an `error` event). Host rejection is reported via `join_failed`; BLE connection failure during join is reported via `error` with code `join_failed` (Section 6.3 step 8d, Section 13 step 7c).
...
REQUIREMENT: The implementation MUST ensure that OnClientDisconnected() is only
invoked for the device currently associated with the active client connection.
Disconnect callbacks for stale devices (from previous connections, prior to
StopClientOnly() cleanup) MUST be discarded at the callback routing level. This
may be achieved by comparing the callback's device handle against the current
client GATT reference, or by any equivalent mechanism that produces the same
filtering.

REQUIREMENT: On platforms where GATT connection failure may be delivered as both
a connection failure event and a subsequent disconnect callback, the
implementation MUST ensure that the disconnect decision tree does not process
the same underlying failure event twice. This may be achieved by a guard flag,
by callback-level deduplication, by thread serialization that allows the first
handler to modify state before the second fires, or by any equivalent mechanism.
...
FUNCTION OnClientDisconnected() -> void
  ...
  2. Derive shouldEmit:
     ...
     2b. Else -> shouldEmit = true.
  3. Call StopClientOnly().
  4. If migrationInProgress is true:
     4a. Call BeginMigrationReconnect(). Return.
  5. If shouldEmit is true AND wasJoined is true:
     5a. Call BeginClientReconnect().
...
**Key invariants:**
- `wasJoined` and `shouldEmit` are captured *before* `StopClientOnly()` modifies state (steps 1-2 before step 3).
- The `shouldEmit` derivation is `NOT clientLeaving`. Stale disconnect callbacks are handled by the callback-routing REQUIREMENT above, not by the `shouldEmit` derivation.
- Steps 7 and 8 call `FinishLeaveClient()` (Section 6.6), not full `FinishLeave()`.
- Disconnects during intentional leave are suppressed (step 2a).
